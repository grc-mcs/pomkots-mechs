...
import grcmcs.minecraft.mods.pomkotsmechs.entity.monster.boss.legacy.goal.WalkBossGoal;
...
//            be.shootFromRotation(be, 0, 0, this.getFallFlyingTicks(), 0F, 0F);
    ...
    private void attackGatling() {
        if (isServerSide()) {
            ...
            GrenadeEntity be = new GrenadeEntity(PomkotsMechs.GRENADE.get(), this.level(), this, 2, 10);
    ...
    private void attackGrenade() {
        if (isServerSide()) {
            ...
            GrenadeLargeEntity be = new GrenadeLargeEntity(PomkotsMechs.GRENADELARGE.get(), this.level(), this, 8);
