...
public class Pmv01EntityRenderer extends GeoEntityRenderer<Pmv01Entity> {

    private static final Logger LOGGER = LoggerFactory.getLogger(PomkotsMechs.MODID);

    public Pmv01EntityRenderer(EntityRendererProvider.Context renderManager) {
        this(renderManager, new Pmv01EntityModel());
    }

    public Pmv01EntityRenderer(EntityRendererProvider.Context renderManager, Pmv01EntityModel model) {
        super(renderManager, model);
        addRenderLayer(new AutoGlowingGeoLayer<>(this));
    }

    @Override
    public void actuallyRender(PoseStack poseStack, Pmv01Entity animatable, BakedGeoModel model, RenderType renderType,
    ...
    private Vec3 getSeatPosition(BakedGeoModel model) {
        if (model != null) {
            ...
            return new Vec3(
                    seatPos.x * Pmv01Entity.DEFAULT_SCALE - rootPos.x * Pmv01Entity.DEFAULT_SCALE,
                    seatPos.y * Pmv01Entity.DEFAULT_SCALE - rootPos.y * Pmv01Entity.DEFAULT_SCALE - 2.8F,
                    seatPos.z * Pmv01Entity.DEFAULT_SCALE - rootPos.z * Pmv01Entity.DEFAULT_SCALE);
        } else {
            return Vec3.ZERO;
        }
    }
    ...
    @Override
    public void preApplyRenderLayers(PoseStack poseStack, Pmv01Entity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        this.scaleHeight = Pmv01Entity.DEFAULT_SCALE;
        this.scaleWidth = Pmv01Entity.DEFAULT_SCALE;
    ...
    private void setBoosterVisibilitty(BakedGeoModel model, Pmv01Entity ent) {
