...
public class CoreStonePMV03Item extends Item {
    // 使用済みフラグのキー
    private static final String USED_KEY = PomkotsMechs.MODID + ":corestone:used";

    public CoreStonePMV03Item(Properties properties) {
        super(properties);
    }
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (target instanceof Pmv03Entity robot) {
            // ロボットの体力を回復
            robot.heal(20.0F);
    ...
    private void summonRobot(Level world, Player player) {
        Pmv03Entity robot = createInstance(world);
        robot.moveTo(player.getX(), player.getY(), player.getZ(), player.getYRot(), player.getXRot());
        world.addFreshEntity(robot);
    }

    protected Pmv03Entity createInstance(Level world) {
        return new Pmv03Entity(PomkotsMechs.PMV03.get(), world);
    }
}
